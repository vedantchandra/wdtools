...
Say we have an un-normalized spectrum from the Sloan Digital Sky Survey (SDSS), defined by wavelengths ``wl``, fluxes ``fl``, and an inverse variance mask ``ivar``. 
...
.. code-block:: python
   ...
   sp = SpecTools()
   mcfit = MCFit(resolution = 3) # Gaussian kernel in Angstrom
